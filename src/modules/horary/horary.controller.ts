...
export class HoraryController {
  ...
  ): Promise<Paginated<HoraryEntity>> {
  ...
  @Post()
  async post(@Body() data: HoraryDto): Promise<HoraryDto | any> {
  ...
  @MessagePattern({ cmd: 'horaryView' })
  async horaryView(id: number): Promise<HoraryDto | any> {
    return this.service.horaryView(id);
  }
}
