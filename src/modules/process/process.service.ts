...
export class ProcessService {
  ...
  async purchasingProcess(data: PurchasingProcessDto) {
    ...
    try {
      ...
      const seatingCapacity = await this.entityManager
        .createQueryBuilder(TicketEntity, 'ticket')
        .leftJoinAndSelect(
          HoraryEntity,
          'horary',
          'ticket.horaryId = horary.id',
        )
        .leftJoinAndSelect(
          VehicleEntity,
          'vehicle',
          'horary.vehicle = vehicle.id',
        )
      ...
      const dataSeats = await this.entityManager
        .createQueryBuilder(TicketEntity, 'ticket')
        .leftJoinAndSelect(
          HoraryEntity,
          'horary',
        ...
        .andWhere('horary.day = :day', {
          day: moment(dateOfPurchase, 'YYYY-MM-DD').format('dddd'),
        })
        .andWhere('vehicle.seatsAvailable >= :numberOfSeats', { numberOfSeats })
        .getMany();
      ...
      let result = null;
      if (user) {
        const userExistence = await this.entityManager
          .createQueryBuilder(TicketEntity, 'ticket')
        ...
        if (userExistence.length === 0) {
          ...
          try {
            newUsert = await lastValueFrom(
              this.catalogoProxy.send({ cmd: 'usercreation' }, user),
            );
          } catch (error) {
